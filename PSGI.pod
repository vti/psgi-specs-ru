...
среду выполнения, и возвращает ссылку на массив, содержащий ровно три
значения.
...
=head3 Среда выполнения

Среда выполнения B<ДОЛЖНА> быть ссылкой на хеш, включащий в себя CGI-подобные
заголовки, как описано ниже. Приложение может свободно изменять среду. Среда
B<ДОЛЖНА> содержать эти переменные (заимствовано из L<PEP
...
Если переменная среды является логической переменной, её значение B<ОБЯЗАНО>
...
Если этот ключ не пуст, он B<ДОЛЖЕН> начинаться с прямого слэш (C</>).
